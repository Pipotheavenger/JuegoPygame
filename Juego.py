...
#Created by Sebastian Valencia, Colombia
pygame.init()
...
class Pokemon(pygame.sprite.Sprite):
    ...
    def perform_attack(self, other, move):
        ...
        display_message(f'{self.name} ha usado {move.name}')
...
level = 30
bulbasaur = Pokemon('Pikachu', level, 25, 100)
charmander = Pokemon('Rattata', level, 175, 100)
squirtle = Pokemon('Gengar', level, 325, 100)
Psyduck = Pokemon('Psyduck', level, 25, 250)
Flareon = Pokemon('Flareon', level, 175, 250)
Electrode = Pokemon('Electrode', level, 325, 250)
pokemons = [bulbasaur, charmander, squirtle,Psyduck,Electrode,Flareon]
...
while game_status != 'quit':
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_y:
                ...
                charmander = Pokemon('Rattata', level, 175, 100)
                squirtle = Pokemon('Gengar', level, 325, 100)
                Psyduck = Pokemon('Psyduck', level, 25, 220)
                Flareon = Pokemon('Flareon', level, 175, 220)
                Electrode = Pokemon('Electrode', level, 325, 220)
                pokemons = [bulbasaur, charmander, squirtle,Psyduck,Electrode,Flareon]
                game_status = 'select pokemon'
            ...
            # for selecting fight or use potion
                ...
                if potion_button.collidepoint(mouse_click):
                    ...
                    if player_pokemon.num_potions == 0:
                        display_message('No quedan mas pociones')
                        time.sleep(2)
                        game_status = 'player move'
                    else:
                        player_pokemon.use_potion()
                        display_message(f'{player_pokemon.name} ha usado una pocion')
                        time.sleep(2)
    ...
    if game_status == 'select pokemon':
        ...
        text = font.render('¡ Selecciona un pokemon para competir !', True,black)
        text_rect = text.get_rect()
        text_rect.topleft = (70, 25)
        game.blit(text, text_rect)
    ...
    if game_status == 'start battle':
        ...
        while alpha < 255:
            ...
            display_message(f'El rival ha escogido {rival_pokemon.name}!')
            alpha += .4
        ...
        while alpha < 255:
            ...
            display_message(f'Sal ahora {player_pokemon.name}!')
            alpha += .4
    ...
    if game_status == 'player turn':
        ...
        fight_button = create_button(240, 140, 10, 350, 130, 412, 'Combate')
        potion_button = create_button(240, 140, 250, 350, 370, 412, f'Usar pocion ({player_pokemon.num_potions})')
    ...
    if game_status == 'fainted':
        ...
        while alpha > 0:
            ...
            if rival_pokemon.current_hp == 0:
                player_pokemon.draw()
                rival_pokemon.draw(alpha)
                display_message(f'{rival_pokemon.name} Ha sido derrotado!')
            else:
                player_pokemon.draw(alpha)
                rival_pokemon.draw()
                display_message(f'{player_pokemon.name} Ha sido derrotado!')
            alpha -= .4
    ...
    if game_status == 'gameover':
        ...
        display_message('Quieres jugar de nuevo (Y/N)?')
